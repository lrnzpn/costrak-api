...
export const getExpenseSummary = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { data, error } = await supabase
      ...
      `)
      .eq('user_id', userId)
      .gte('date', start_date || new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().split('T')[0])
    ...
    const summary = data.reduce((acc: Record<string, { id: string, name: string, total: number }>, item: any) => {
